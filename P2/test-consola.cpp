...
int main()
{
  ...
  try {		 // O398 en vez de 0398 ;-) (era una O, no un cero)
    Tarjeta falsa(Numero("4539 4512 O398 7356"), lucas, Fecha(31, 12, Fecha().año() + 3));
  }
  catch(Numero::Incorrecto& e) {
    numero_incorrecto(e);
  }
  try {
    Tarjeta judas(Numero("1234567890123456"), lucas, Fecha(31, 12, Fecha().año() + 3));
  }
  catch(Numero::Incorrecto& e) {
    numero_incorrecto(e);
  }
  try {
    Tarjeta mala(Numero("123"), lucas, Fecha(31, 12, Fecha().año() + 4));
  }
  catch(Numero::Incorrecto& e) {
    numero_incorrecto(e);
  }
  try {
    Tarjeta caduca(Numero("4222222222222"), lucas, Fecha(31, 12, Fecha().año() - 1));
  }
  catch(Tarjeta::Caducada &e) {
    cerr << "Error: Tarjeta caducada desde el " << e.cuando() << endl;
  }
  try { 			// En la fecha, O = letra O, no cero (O-0)
    Tarjeta rota(Numero(" 4222222222222 "), lucas, "1O/O4/2O28");
  }
  ...
  Autor 
    ...
    Verde("Vaina", "Verde", "Pedroborron (US)"), 
    Telita("Aguja", "Cosida", "Offenburg (Hermania)");
...
//      "220", "Referencia de C++", "9/9/2020", 12.90, 547, 30);
  ...
  //-------------- Revistas
  // Constructor de Revista: autores, referencia, título, fecha publicación,
  //                          precio, numero, periodicidad, stock
  autores.clear();
  autores.insert(&Verde);
  Revista byter(autores,	// autor = Verde
      "701", "Byter", "15/8/2022", 15.70, 101, 30, 12);
  autores.insert(&Telita);
  Revista zurza(autores,	// autores = Verde y Telita
      "711", "Compu y Tadora Zurza", "12/3/2018", 5.95, 512, 15, 20);
  autores.insert(&Esteban);
  Revista secpp(autores,	// autores = Verde, Telita y Esteban
      "752", "Secretos de C++", "19/9/2017", 12.90, 27, 30, 8);
#else
...
//    cppref("220", "Referencia de C++", "9/9/2020", 12.90, 30),
    byter("701", "Byter", "15/8/2022", 15.70, 12), 
    zurza("711", "Compu y Tadora Zurza", "12/3/2018", 5.95, 20), 
    secpp("752", "Secretos de C++", "19/9/2017", 12.90, 8), 
...
#ifndef P2
  // Factura: Usuario_Pedido, Pedido_Articulo, Usuario, Numero, Fecha [hoy]
...
//  lucas.compra(adenda, 1);
  lucas.compra(zurza, 1);
  mostrar_carro(cout, lucas);
#ifndef P2
  Pedido p1(usuario_ped, pedido_art, lucas, visa1.numero(), Fecha(10, 3));
#endif
...
//  krispin.compra(cppref);
  ...
  mostrar_carro(cout, yoshua);
#ifndef P2
  Pedido p2(usuario_ped, pedido_art, yoshua, american_express.numero(), 
	    --Fecha());	// pedido ayer
  Pedido p3(usuario_ped, pedido_art, krispin, mastercard.numero(), Fecha(5, 4, Fecha().año()-1));
#endif
  ...
  yessi.compra(secpp, 2);
//  yessi.compra(cppref, 2);
  yessi.compra(tonto, 3);
  yessi.compra(salem);
//  yessi.compra(adenda, 0); // no hace nada, adenda no está en el carro
  yessi.compra(zurza, 0); // no hace nada, zurza no está en el carro
  mostrar_carro(cout, yessi);
#ifndef P2
  try {
    Pedido p4(usuario_ped, pedido_art, yessi, jcb.numero(), Fecha(5, 4, Fecha().año()-1));
  }
  catch(Pedido::Impostor) {
    cout << "Yessi ha intentado pagar con una tarjeta que ya no tiene." << endl;
  }
  Pedido p4(usuario_ped, pedido_art, yessi, dinners_club.numero(), Fecha(5, 4, Fecha().año()-1));
  cout << AZUL "\n---------------------PEDIDOS-----------------------\n"
  ...
  Pedido p5(usuario_ped, pedido_art, yessi, dinners_club.numero()); // pedido hoy
  ...
  Pedido p6(usuario_ped, pedido_art, yessi, dinners_club.numero()); // pedido hoy
  ...
  Pedido p7(usuario_ped, pedido_art, krispin, australian_bank_card.numero(), Fecha(5, 4, Fecha().año()-1));
  cout << VERDE "*** Yoshua compra 2 artículos";
  yoshua.compra(enrique_alfarero, 2); 
  yoshua.compra(manualidades1);
  cout << " y hace su pedido.\n" NORMAL;
  Pedido p8(usuario_ped, pedido_art, yoshua, american_express.numero(), Fecha(5, 4, Fecha().año()-1));
  ...
  cerr << "\n" AZUL "---------PRUEBAS DE EXCEPCIONES DE PEDIDOS---------"
    NORMAL "\n";
  try {
    Pedido nulo(usuario_ped, pedido_art, lucas, visa2.numero());
  }
  ...
  try {
    ...
    Pedido farsante(usuario_ped, pedido_art, krispin, visa2.numero());
  }
  ...
  try {
    ...
    Pedido no_disponible(usuario_ped, pedido_art, yessi, dinners_club.numero());
  }
  ...
  try {
    ...
    Pedido no_pagado(usuario_ped, pedido_art, lucas, caduca.numero()); // pedido hoy
  }
  catch(Tarjeta::Caducada& e) {
