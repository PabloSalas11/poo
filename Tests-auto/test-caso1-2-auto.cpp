...
namespace {
  ...
  const Fecha  fSiguienteAño(1, 1, fHoy.año() + 1);
#ifdef P4
...
FCTMF_FIXTURE_SUITE_BGN(test_p2) {
  ...
  FCT_TEST_BGN(Clave - observador y cifrado DES) {
    const Clave c("!.:xA?");
    fct_chk_neq_str(c.clave().operator const char*(), "!.:xA?");
...
#ifndef P2 // Se pide en P3, no en P2
    const Numero::EsDigito digito;
    fct_xchk(digito('5'), "Dígito no detectado");
    fct_xchk(!digito('z'), "Detectada letra como dígito");
#endif
    try {
  ...
  FCT_TEST_BGN(Tarjeta - duplicada) {
    ...
    try {
      Tarjeta tarjeta2(nTarjeta, *pU, fSiguienteAño);
  ...
  FCT_TEST_BGN(Tarjeta - insercion en flujo) {
    const Tarjeta tarjeta(nTarjeta, *pU, fSiguienteAño);
    ...
    // Equivalente en C++ a printf("%02d/%02d", mes, año%100);
    ostringstream os;
    os << setfill('0') << setw(2) << tarjeta.caducidad().mes()
       << '/' << setw(2) << (tarjeta.caducidad().año() % 100);
  ...
  FCT_TEST_BGN(Usuario---Tarjeta - insercion en flujo) {
    ...
    chk_incl_cstr(s, (const char *)(sId));
    chk_incl_cstr(s, (const char*)(clave.clave()));
    chk_incl_cstr(s, (const char*)(sNombre));
    chk_incl_cstr(s, (const char*)(sApellidos));
    chk_incl_cstr(s, (const char*)(sDireccion));
