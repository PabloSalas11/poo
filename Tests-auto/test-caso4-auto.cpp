...
//    pUsuarioPedido->compra(cdrom, 1);
    ...
    thread_local Pedido p(usuPed, pedArt, *pUsuarioPedido, pTarjetaPedido->numero());
...
//    fct_chk_eq_dbl(p.total(), cdrom.precio());
  ...
  FCT_TEST_BGN(LibroDigital - eBook expirado y carrito queda vacio) {
    pUsuarioPedido->compra(eBookExpirado, 1);
    fct_chk_ex(Pedido::Vacio, Pedido(usuPed, pedArt, *pUsuarioPedido, pTarjetaPedido->numero()));
  }
  FCT_TEST_END();
