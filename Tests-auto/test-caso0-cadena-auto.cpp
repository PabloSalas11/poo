...
// modificadoras que pueden lanzar bad_alloc.
  ...
  FCT_TEST_BGN(Cadena - Asignar una copia: memoria no disponible) {
    rlimit ant, nuevo{1024 * 1024, 1024 * 1024}; // máx. memoria disponible 1 MiB
    if (prlimit(0, RLIMIT_DATA, &nuevo, &ant) == 0) {
      size_t i = 0;
      const char* d;
      vector<Cadena> vcad(1024, Cadena(3, '+'));
      try {
        while (i < vcad.size()) {
          d = vcad[i].operator const char*();
          vcad[i] = Cadena(1023, '@');
          ++i;
        }
        cerr << "\n   Test no realizado: memoria no agotada.\n";
      } catch (bad_alloc&) {
        for (size_t j = 0; j < i; ++j) vcad[j] = Cadena(); // Liberar mem. para el test
        fct_xchk(strcmp(vcad[i].operator const char*(), "+++") == 0, 
                  "Asignación de copia: memoria insuficiente ('std::bad_alloc'). "
                  "La cadena destino no debería cambiar de contenido.");
        fct_xchk(vcad[i].length() == 3,
                  "Asignación de copia: memoria insuficiente ('std::bad_alloc'). "
                  "La cadena destino no debería cambiar de longitud.");
        fct_xchk(vcad[i].operator const char*() == d,
                  "Asignación de copia: memoria insuficiente ('std::bad_alloc'). "
                  "la dirección del operador const char*() de la "
                  "cadena no debería cambiar.");
      }
...
// modificadoras que pueden lanzar bad_alloc.
  ...
  FCT_TEST_BGN(Cadena - Asignar una cadena de bajo nivel: memoria no disponible) {
    ...
    if (prlimit(0, RLIMIT_DATA, &nuevo, &ant) == 0) {
      ...
      try {
        while (i < vcad.size()) {
          c = vcad[i].operator const char*();
          vcad[i] = Cadena(1023, '@').operator const char*();
          ++i;
        }
        cerr << "\n   Test no realizado: memoria no agotada.\n";
      } catch (bad_alloc&) {
        ...
        fct_xchk(vcad[i].length() == 3,
                  ...
                  "('std::bad_alloc'). La cadena destino no debería cambiar de "
                  "longitud.");
        fct_xchk(vcad[i].operator const char *() == c,
                  "Asignación de cadena de bajo nivel: memoria insuficiente "
...
// modificadoras que pueden lanzar bad_alloc.
  ...
  FCT_TEST_BGN(Cadena - Subcadena: tamaño cero) {
    const Cadena a("eenie meenie moe");
  ...
  FCT_TEST_BGN(Cadena - Subcadena: tamaño mayor que longitud restante) {
    const Cadena a("0123456789");
  ...
  FCT_TEST_BGN(Cadena - Subcadena: tamaño negativo) {
    const Cadena a("0123456789");
