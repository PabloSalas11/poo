...
FCTMF_FIXTURE_SUITE_BGN(test_p3_clases) {
  ...
  FCT_TEST_BGN(Pedido - carrito vacio) {
    ...
    try {
      Pedido {*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU, nTarjeta, fHoy};
  ...
  FCT_TEST_BGN(Pedido - impostor) {
    ...
    try {
      Pedido {*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU2, nTarjeta, fHoy};
  ...
  FCT_TEST_BGN(Pedido - tarjeta caducada) {
    ...
    try {
      Pedido {*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU, nTarjeta, fHoy + 30};
  ...
  FCT_TEST_BGN(Pedido - tarjeta desactivada) {
    ...
    try {
      Pedido(*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU, nTarjeta, fHoy);
      fct_xchk(false, "Se esperaba una excepción Tarjeta::Desactivada");
    }
  ...
  FCT_TEST_BGN(Pedido - actualizacion relacionados y asociaciones) {
    ...
    const unique_ptr<Pedido> pPed {
      new Pedido {*pAsocUsuarioPedido, *pAsocPedidoArticulo, *pU, nTarjeta} 
    };
    ++N_pedidos;
