...
//         int f1(int i) const;
//         int f2(int i);
//         static void f3() {m = h;} //Error al ser una funcion estatica y no recibir el puntero this no se puede acceder a los miembros no estaticos, por lo que se debe cambiar a static void f3() {m = 0;} o se le quita el static a la funcion
//         static int n;
//     private:
//         int h;
//         mutable int i; //si se modifica en una funcion const, al ser mutable, se puede modificar
...
// C::C() {h=i=j=k=0;} //Error 2: j es cont y se modifica dentro del cuerpo del constructor, se debe inicializar en la lista de inicializacion del constructor, por lo que se debe cambiar a C::C(): h{0}, j{0}, k{0} {}
// C::C(int a, int b, int c, int d): h{a}, j{b}, m{c}, k{d} {}
// int C::f2(int i){
//     k=i; h=i; return 0;
// }